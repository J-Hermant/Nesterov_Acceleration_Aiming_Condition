...
def run_continuized_nest(init,n_iter,gamma,gamma_p,eta,eta_p,t,T,D,L,y,x_ast):
    ...
    for i in range(n_iter-1):
        ...
        mu[i] = 0.5*norm(grad_cna)**2/(g_tilde(y_nm,t,T,D,L,y))
        corr_nag[i] = np.sum(grad_cna*(y_nm-x_ast))/(norm(grad_cna)*norm(y_nm-x_ast))
...
fontsize_legend = 16
fontsize = 14
plt.plot(np.log(f_nag),lw=3,label = r"$log(f)$ using NM")
plt.plot(np.log(f_gd),lw=3, label =r"$log(f)$ using GD")
plt.plot(np.arange(n_iter),np.arange(1,n_iter+1)*np.log(1-(mu_gd.mean()/L)*13)+np.log(f_gd[0]),lw=3,label="Bound (11) for GD")
plt.plot(np.arange(n_iter),np.arange(1,n_iter+1)*np.log(1-(24/80)**(1/4)*corr_nag.mean()*np.sqrt((mu.mean()/L)*9))+np.log(f_nag[0]),lw=3,label=("Bound (12) for NM"))
plt.plot(np.arange(n_iter),np.arange(1,n_iter+1)*np.log(1-(10**(-4)/L)*13)+np.log(f_gd[0]),lw=3,label ="Theoretical worst case bound for GD")
